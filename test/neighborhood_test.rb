...
class NeighborhoodTest < MiniTest::Unit::TestCase
  NEIGHBORHOOD = Neighborhood.new([[1, 1, 0],
                                   [0, 1, 0],
                                   [1, 1, 0]])
  ...
  describe NEIGHBORHOOD do
    it "should die with fewer than two live neighbors" do
      assert_equal 0, Neighborhood.new([[1, 0, 0],
                                        [0, 1, 0],
                                        [0, 0, 0]]).tick
    end

    it "should survive with two or three live neighbors" do
      assert_equal 1, Neighborhood.new([[1, 1, 0],
                                        [0, 1, 0],
                                        [0, 0, 0]]).tick

      assert_equal 1, Neighborhood.new([[1, 0, 0],
                                        [1, 1, 0],
                                        [1, 0, 0]]).tick
    end

    it "should die with more than three live neighbors" do
      assert_equal 0, Neighborhood.new([[1, 0, 1],
                                        [1, 1, 0],
                                        [1, 0, 0]]).tick

      assert_equal 0, Neighborhood.new([[1, 1, 1],
                                        [1, 1, 1],
                                        [1, 1, 1]]).tick
    end

    it "should be born with exactly three live neighbors" do
      assert_equal 1, Neighborhood.new([[0, 1, 0],
                                        [1, 0, 1],
                                        [0, 0, 0]]).tick
